...
class OpenRouterAPIWithMCP(OpenRouterAPI):
    ...
    def _get_model_capabilities(self, model: str) -> ModelCapabilities:
        ...
        capabilities = ModelCapabilities(
            ...
            supports_streaming=not ModelConfigurations.model_has_streaming_tool_conflict(model),
            has_streaming_tool_conflict=ModelConfigurations.model_has_streaming_tool_conflict(model),
            provider=provider,
    ...
    def _is_circuit_breaker_open(self) -> bool:
        ...
        if current_time - self._circuit_breaker_last_failure_time > self.circuit_breaker_timeout:
            self._circuit_breaker_open = False
            self._circuit_breaker_failure_count = 0
    ...
    def _handle_circuit_breaker_failure(self) -> None:
        ...
        if self._circuit_breaker_failure_count >= self.circuit_breaker_threshold:
            ...
            self.logger.warning(f"Circuit breaker opened after {self._circuit_breaker_failure_count} failures")
    ...
    def _resolve_model_and_capabilities(self, model: Optional[str]) -> Tuple[str, ModelCapabilities]:
    ...
    async def _get_mcp_tools_for_model(self, capabilities: ModelCapabilities) -> List[Dict[str, Any]]:
    ...
    def _has_known_conflicts(
        ...
        return (
            model in self._tool_unsupported_models or
            (mcp_tools and capabilities.has_streaming_tool_conflict) or
            model in self._streaming_conflict_models
        )
    ...
    async def _fallback_to_standard_generation(
        ...
        self.logger.info(f"Using standard generation for {model} due to known conflicts")
...
"""
    ...
    async def generate_response_with_tools(
        ...
        try:
            ...
            return await self._handle_tool_calling_error(e, model, prompt, context, temperature, max_tokens, timeout)
    ...
    async def _execute_tool_calling_workflow(
        ...
        if self._has_tool_calls(response):
            return await self._process_tool_calls(response, messages, openrouter_model, temperature, max_tokens, timeout)
        else:
    ...
    async def _process_tool_calls(
        ...
        return await self._generate_final_response(messages, model, temperature, max_tokens, timeout)
    ...
    async def _generate_final_response(
        ...
        final_adapted_params = ModelConfigurations.adapt_request_for_model(model, final_request_params)
    ...
    def _log_categorized_error(self, error_type: ErrorType, model: Optional[str], error_str: str) -> None:
